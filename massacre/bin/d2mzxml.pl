...
for($ctr_d = 0; $ctr_d <= $#ls_d; $ctr_d++)
{
	...
	if(!(-e $mzml))
	{
		...
		$out3 = `ssh -p 2222 mass\@vaginicola.scripps.edu "pwiz/msconvert.exe //VBOXSVR/vbox_ms/$dir/$ls_d[$ctr_d] --mzXML --filter 'msLevel 1' --mz32 --inten32 --filter 'mzWindow [400,2000]' --o //VBOXSVR/vbox_ms/$dir --gzip"`;
		print "$out3\n";
