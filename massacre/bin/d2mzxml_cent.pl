...
for($ctr_d = 0; $ctr_d <= $#ls_d; $ctr_d++)
{
	...
	if(!(-e $mzml))
	{
		...
		#
		print "Generating mzXML file\n";
		$out3 = `ssh -p 2222 mass\@vaginicola.scripps.edu "pwiz/msconvert.exe //VBOXSVR/vbox_ms/$dir/$ls_d[$ctr_d] --mzXML --filter 'peakPicking true 1-' --mz32 --inten32 --filter 'mzWindow [200,2000]' --o //VBOXSVR/vbox_ms/$dir --gzip"`;
		print "$out3\n";
		print "Done generating mzXML file\n";
		
		#
		print "Copying mzXML file back\n";
