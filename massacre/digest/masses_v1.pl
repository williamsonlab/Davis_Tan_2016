...
# Input Header:
# seq mod missed num_copy loc permutation
#
...
LINE: while(defined($input=<STDIN>))
{
	...
	#
	#
	# loc -> location, as comma-separated array: protein,start_aa,end_aa,protein,start_aa,end_aa,etc
	#
	@array = split ' ', $input;
	...
	#
	# Parse out the protein location information
	# If num_copy == 1, then we simply keep the current information
	#
	if($num_copy == 1)
	{
		@loc_array = split /\,/, $locationstring;
	}
	#
	# Two copies - often this is an A/B pair, so we check this option
	# Also check to see if both copies come from the same protein/RNA
	#
	elsif($num_copy == 2)
	{
		@temp_loc_array = split /\,/, $locationstring;

		# Check to see if this is an A/B protein pair -> S4A and S4B or whatever
		$prota = $temp_loc_array[0];
		$protb = $temp_loc_array[3];
		...
		$ida = substr($prota, -1, 1);
		$idb = substr($protb, -1, 1);
		
		$ab = 0;
		# Is the protein root (everything but last digit) the same?
		if($suba eq $subb)
		{
			# Is the last digit A and B?
		...
		if($ab == 1)
		{
			$prot_out = join '', $suba, "X";
			# Check to see if the start/end numbers are the same
			# If they are keep them
			if($temp_loc_array[1] == $temp_loc_array[4] && $temp_loc_array[2] == $temp_loc_array[5])
			{
				@loc_array = ( $prot_out, $temp_loc_array[1], $temp_loc_array[2] );
			}
			# If they are not, use 0 -> 0
			else
			{
				@loc_array = ( $prot_out, "0", "0" );
			}
		}
		else
		{
			# Not an A/B pair, but is it the same protein (just different locations?)
			if($prota eq $protb)
			{
				@loc_array = ( $prota, "0", "0" );
			}
			# Not the same protein, so it becomes XXX
			else
			{
				@loc_array = ( "XXX", "0", "0" );
			}
	...
	#
	# If num_copy > 2, we check to see if all copies are from the same protein or not
	# If yes, the array becomes ("protein", "0", "0"), otherwise ("XXX", "0", "0")
	#  
	else
	{
		@temp_loc_array = split /\,/, $locationstring;
		$num_redun = ($#temp_loc_array + 1) / 3;
		
		$match = 1;
		# Check protein IDs and compare them to that of the first peptide
		for($ctr_redun = 1; $ctr_redun < $num_redun; $ctr_redun++)
		{
			# If the proteins don't match, set $match to false
			if($temp_loc_array[3*$ctr_redun] ne $temp_loc_array[0])
			{
				$match = 0;
			}
		}
		...
		{
			@loc_array = ($temp_loc_array[0], "0", "0");
		}
	}
	
	if($#loc_array != 2)
	{
		die "Error setting location array for $input\n";
	}
	...
	CHARGE: for($ctr = $min_charge; $ctr <= $max_charge; $ctr++)
	{
		...
		$peptide_line = join ' ', $mz, $mz_n15, @loc_array, $iontype, $ctr, $missed, $outseq, $mod;
...
# Print
#
print "digestionid digestpepid n14mass n15mass protein startres endres iontype charge missed seq mod ";
if($output_avg == 1)
{
